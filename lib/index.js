...
/**
 * Splits a stream into batches when truth test passes or end of
 * stream is reached. Discards the value split on.
 *
 * @id split
 * @section Streams
 * @name Stream.split(f)
 ...
 * @api public
 *
 * var evens = _([1, 2, 3, 4, 5]).filter(function (x) {
 *     return x === 3
 * });
 * // => [1,2] [4,5]
 */

Stream.prototype.split = function (f) {
    var batch = [];
    return this.consume(function (err, x, push, next) {
        if (err) {
            push(err);
...
/**
 * A moving window over the stream which returns arrays of length n. 
 *
 * @id window
 * @section Streams
 * @name Stream.window(n)
 * @param {Number} n - number of elements in window
 * @api public
 *
 * _([1, 2, 3, 4, 5]).window(2)  // => [1, 2], [2, 3], [3, 4], [4, 5]
 */
